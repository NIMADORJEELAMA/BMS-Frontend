...
const checkInSchema = z.object({
  ...
  checkInDate: z.string().min(1, "Date required"),
  checkOutDate: z.string().min(1, "Departure required"),
...
export default function CheckInModal({
  ...
  useEffect(() => {
    if (isOpen && preselectedData) {
      form.setValue("roomId", preselectedData.roomId);

      // Format date for the input: YYYY-MM-DDTHH:mm
      const dateStr = preselectedData.date.toISOString().slice(0, 16);
      form.setValue("checkInDate", dateStr);
    }
  }, [isOpen, preselectedData, form]);
